...
Kbase.init = function() {
    ...
    function(indexpage){
        ...
        indexpage = indexpage.substring(0, indexpage.indexOf("</head>") - 1) + "<link rel=\"stylesheet\" type=\"text/css\" href=\"webhelp.css\">" + indexpage.substring(indexpage.indexOf("</head>"));
    ...
    var showCustomerSupportIcon = Kbase._showIt("customerSupportIcon");
	// Font Resizer icon
    var showFontResizerIcon = Kbase._showIt("fontResizerIcon");
	...
	// Show/Hide footer
    var showFooter = Kbase._showIt("showFooter");
    ...
    var headerHeight = 50;
    var ToolbarHeight = 30;//30 was 25 LYS is the height for Toolbar below header (the mainToolbar)
    ...
    var footerHeight = 5;
    //check whether the footer is hidden or not
    if(!showFooter){//Hide footer
        footerHeight = 0;
    }else {
        //set the footer height dynamically from whatever set in css i.e. (div#footer { height: 20px; ...... } 
        if(document.getElementById("footer")){
            var footerH = Util.getStyle(document.getElementById("footer"), "height");
            if(footerH){
                var fh = parseInt(footerH);
                if(!isNaN(fh))
                    footerHeight = fh;
            }
        }
    ...
    this._searchField = new Ext.form.TextField({id: 'search-textfield', width: 250, emptyText: Kbase.GetLocalizedResource("Search")})
	...
	var fontSizeOptionsMenu = new Ext.menu.Menu({
        id: 'fontSizeOptionsMenu',
        
        items: 
		[	
			{
				text: '<span style="font-size: 9px">' + Kbase.GetLocalizedResource("Small") + '</span>', 
				value: 'small-fonts',
				checked: false,
				group: 'fontsizeoptions',
				id: 'small-fonts',
				checkHandler: this.onFontResizerOptionChecked
			}, 
			{
				text: '<span style="font-size: 10px">' + Kbase.GetLocalizedResource("Medium") + '</span>', 
				value: 'medium-fonts',
				checked: true,
				group: 'fontsizeoptions',
				id:'medium-fonts' ,
			...
			{
				text: '<span style="font-size: 13px">' + Kbase.GetLocalizedResource("Large") + '</span>', 
				value: 'large-fonts',
				checked: false,
				group: 'fontsizeoptions',
				id: 'large-fonts' ,
                ...
                items: [
                    ...
                    new Ext.Toolbar({//Toolbar
                        ...
                        items: [ ' ', ' ',
							...
							,' ',' ',
							{
                                hidden: (!showFontResizerIcon),
                                iconCls: 'fontResizerIcon',
			                    tooltip: Kbase.GetLocalizedResource("Font Size"),
			                    tooltipType: 'title',
								id:'fontResizerOptionsMenu',
			                    menu: fontSizeOptionsMenu
                            }
							,' ',' ',
                             {
                                hidden: (!showCustomerSupportIcon),
                                iconCls: 'customerSupportIcon',
			                    tooltip: Kbase.GetLocalizedResource("Customer Support"),
			                    tooltipType: 'title', 
			                    scope: this,
			                    handler: this._onClick_customerSupport_icon
                            }
							,' ',' ',
                             {
	...
	Kbase._initializeFontStyleSheet();
};
...
Kbase._F_openTopicById = function(topicId) {
    ...
    if(topicNode) {
        Util.ensureVisibleWithoutScollingRight(topicNode, Kbase._tocTree.getEl().dom.childNodes[0].childNodes[0]);
...
// Build HTML to populate IFrame with for printing
	var printHTML = "<html><body style='overflow: scroll'>"+ links + DocumentContainer.parentNode.innerHTML + "</body></html>";
	
	// Using Frame so that we can show print from the window without 
...
// Customer Support
Kbase._onClick_customerSupport_icon = function() {
    var customerSupportPageUrl = Kbase._baseUrl + "index.htm#" + "customer_support.htm";
	
	Kbase._F_openTopicByUrl(customerSupportPageUrl);
};

// Font Resizer Option
Kbase.onFontResizerOptionChecked = function(item, checked) {
	
	if (checked == true)
	{
		var fontSizeOption = item.value;
		Kbase._setActiveFontsStyleSheet(fontSizeOption);
	}
};
...
Kbase._navigateToIndexLetter = function(e, source, letterId) {
	...
	var query = "#index a[name=" + letterId + "]";
    var letterElems = Ext.DomQuery.select(query);
		
	if (letterElems[0] != null)
	{
		var letterElem = letterElems[0];
...
Kbase._searchHighlight = function(id) {
	...
	// Scroll to show the first result
	var searchResultSpanElems = Ext.get('topic-content').select('span[class=searchword1]');
	if (searchResultSpanElems.elements.length > 0)
	{
		var searchResultSpanElem = searchResultSpanElems.elements[i];
        Util.scrollToTop(Ext.get(searchResultSpanElem), Ext.get('topic-content').dom.parentNode.id);
    }
...
Kbase._startSearch = function() {
    ...
    if(searchTerm.length >= 1) {
        ...
        if(resultCount > 0){
            ...
            var t = (t2-t1)/1000; 
            var resultFound = "<p id='searchDetails' class='searchDetails'>" + "<b>Search Results</b><br>" + resultCount + " " + Kbase.GetLocalizedResource("result(s) found") + "<br />" + "</p><br />";
...
Kbase._getCookie = function(name) 
{ 
...
Kbase._setCookie = function(name, value, expires, path, domain, secure)
{ 
    var today = new Date(); 
    if(expires){ 
...
Kbase._deleteCookie = function(name, path, domain)
{ 
    if(getCookie(name)){ 
...
Kbase._setActiveFontsStyleSheet = function(fontSizeOption)
{
	Kbase._setCookie('fontSizeOption', fontSizeOption,30);
	
    var i, linkElement;
  
	for (i = 0; (linkElement = document.getElementsByTagName("link")[i]); i++) 
	{
		var attr = linkElement.getAttribute("rel");
		var ind = attr.indexOf("style");
		var titattr = linkElement.getAttribute("title");
	
		if (linkElement.getAttribute("rel").indexOf("style") != -1 && linkElement.getAttribute("title")) 
		{
			linkElement.disabled = true;
			
			if (linkElement.getAttribute("title") == fontSizeOption)
			{
...
Kbase._initializeFontStyleSheet = function()
{

	//alert(fontSizeOption);
	var fontSizeOption;
	try
	{
		fontSizeOption = Kbase._getCookie("fontSizeOption")
	}
	catch(e){}
	
	if (fontSizeOption == null)
		fontSizeOption = "medium-fonts";
	
	var items = Ext.getCmp("fontResizerOptionsMenu").menu.items.items; 
	
	for (i = 0; (itemElement = items[i]); i++) 
	{
		if (itemElement.id == fontSizeOption)
		{
			itemElement.setChecked(true);
			break;
		}
	}
};
...
Util = {
	...
	// Scroll a container to the top of an element.
    scrollToLeft: function(container) {
        var p = Ext.get(container);
        p.scrollTo("left", 0, true);
    },
	ensureVisibleWithoutScollingRight: function(elem, container) {
		elem.ensureVisible();
		Util.scrollToLeft(container);
	},
